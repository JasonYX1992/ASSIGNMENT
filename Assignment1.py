...
def required_list():    # Build a required list
    if L1:              # If the list is not empty
        s = 0           # s is for sum
        v = 0           # v is for sequence number
        sort_func(L1)   # Build a sorted list according to the priority
        for i in L1:
            s += float(i[1])   # Add up and sum the total number
            print('{0}. {1}         $  {2:.2f} ({3})'.format(v, i[0], float(i[1]), i[2]))
            v += 1
        print('Total expected price for {0} items: ${1:.2f}'.format(len(L1), s))
    else:
...
def completed_list():  # Build a completed list
    if L2:             # If the list is not empty
        ...
        for i in L2:
            ...
            print('{0}. {1}         $  {2:.2f} ({3})'.format(v, i[0], float(i[1]), i[2]))
            v += 1
        print('Total expected price for {0} items: ${1:.2f}'.format(len(L2), s))
    else:
        print('No completed items')
...
def menu():             # Build a menu
    print('''
Shopping List 1.0 - by {0}
{1} items loaded from items.csv'''.format('Yang Xing', len(L1)))
    while True:
        print(
'''Menu:
...
# global L1,L2
csv_file = open('items.csv', newline='')  # Open the csv_file
L1 = list(csv.reader(csv_file))  # Read the data  # print(L1)
csv_file.close()
